...
export const getShape = ({ shape, vertices, indices, options, }) => {
    ...
    const flags = new PhysX.PxShapeFlags(PhysX.PxShapeFlag.eSCENE_QUERY_SHAPE.value |
        PhysX.PxShapeFlag.eSIMULATION_SHAPE.value);
    return PhysXManager.instance.physics.createShape(geometry, material, false, flags);
};
const getGeometry = ({ shape, vertices, indices, options = {}, }) => {
...
const createTrimesh = (cooking, physics, vertices, indices) => {
